...
class ShuffledRowRDD(
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[InternalRow] = {
    ...
    logDebug("ShuffledRowRDD.scala test")
    ...
    val reader = split.asInstanceOf[ShuffledRowRDDPartition].spec match {
      case CoalescedPartitionSpec(startReducerIndex, endReducerIndex) =>
        logDebug("CoalescedPartitionSpec test")
      ...
      case PartialReducerPartitionSpec(reducerIndex, startMapIndex, endMapIndex, _) =>
        logDebug("PartialReducerPartitionSpec test")
        SparkEnv.get.shuffleManager.getReader(
          dependency.shuffleHandle,
          startMapIndex,
          endMapIndex,
      ...
      case PartialMapperPartitionSpec(mapIndex, startReducerIndex, endReducerIndex) =>
        logDebug("PartialMapperPartitionSpec test")
        SparkEnv.get.shuffleManager.getReader(
          dependency.shuffleHandle,
          mapIndex,
          mapIndex + 1,
