...
public class ExternalBlockStoreClient extends BlockStoreClient {
  ...
  /**
   * Informs the external (remote) shuffle server to deregister from it.
   * This function is called right after the driver sends Shutdown Message to the executor.
   * With the de-registration, the shuffle server deletes every shuffle file related to this executor.
   *
   * @param host Host of shuffle server.
   * @param port Port of shuffle server.
   * @param execId This Executor's id.
   */
  public void deregisterFromShuffleServer(
          String host,
          int port,
          String execId){
    checkInit();
    try (TransportClient client = clientFactory.createClient(host, port)) {
      ByteBuffer deregisterMessage = new DeregisterExecutor(appId, execId).toByteBuffer();
      client.send(deregisterMessage); //now sent as OneWayMessage Type. maybe should test sendRpc to send as RpcRequest Type
    } catch (Exception e) {
      logger.error("Exception while sending de-registration message to Shuffle Server, ", e);
    }
  }
