...
private[worker] class WorkerArguments(args: Array[String], conf: SparkConf) {
  ...
  if (System.getenv("SPARK_WORKER_PORT") != null) {
    if (System.getenv("SPARK_WORKER_PORT").contains(":")){
      port = System.getenv("SPARK_WORKER_PORT").substring(System.getenv("SPARK_WORKER_PORT").lastIndexOf(':') + 1).toInt
    }
    else{
  ...
  if (System.getenv("SPARK_WORKER_WEBUI_PORT") != null) {
    if (System.getenv("SPARK_WORKER_WEBUI_PORT").contains(":")){
      webUiPort = System.getenv("SPARK_WORKER_WEBUI_PORT").substring(System.getenv("SPARK_WORKER_WEBUI_PORT").lastIndexOf(':') + 1).toInt
    }
    else{
