...
private class ShuffleStatus(numPartitions: Int) extends Logging {
  ...
  /**
   * Serializes the mapStatuses array into an efficient compressed format for CHERRY shuffle service. See the comments on
   * `MapOutputTracker.serializeMapStatusesForShuffleService()` for more details on the serialization format.
   *
   * This method is designed to be called once for every stage.
   */
  def serializedMapStatusForShuffleService(
    conf: SparkConf): Array[Byte] = {
    var result: Array[Byte] = null

    val serResult = MapOutputTracker.serializeMapStatusesForShuffleService(
      mapStatuses, conf)
    result = serResult
      ...
      try {
        while (true) {
          try {
            ...
            logDebug(s"shuffleStatus: $shuffleStatus")
            logDebug(s"shuffleId requested: $shuffleId")
            logDebug(s"serializedMapStatus INFO FROM MAPOUTPUTTRACKER to send: ${broadcastManager} , ${isLocal} , ${minSizeForBroadcast} , ${conf}")
            logDebug(s"serializedMapStatus FROM MAPOUTPUTTRACKER to send: ${shuffleStatus.serializedMapStatus(broadcastManager, isLocal, minSizeForBroadcast, conf)}")
            logDebug(s"serializedMapStatus NEW FROM MAPOUTPUTTRACKER to send: ${shuffleStatus.serializedMapStatusForShuffleService(conf)}")
            logDebug(s"serializedMapStatus NEW FROM MAPOUTPUTTRACKER mapStatuses to send: ${shuffleStatus.mapStatuses}, size: ${{shuffleStatus.mapStatuses.length}}")
            context.reply(
  ...
  private def getStatuses(shuffleId: Int, conf: SparkConf): Array[MapStatus] = {
    ...
    if (statuses == null) {
      ...
      fetchingLock.withLock(shuffleId) {
        ...
        logTrace(s"MapStatuses: $fetchedStatuses, Current fetched mapStatuses length: ${fetchedStatuses.length}")
        fetchedStatuses
      }
    } else {
      statuses
...
/**
 * Remote_Shuffle_Service-side client for decoupling map output info.
 * The data is already fetched from the driver's MapOutputTrackerMaster.
 */
private[spark] class MapOutputTrackerShuffleService(conf: SparkConf, securityMgr: SecurityManager) extends MapOutputTracker(conf) {
  var serMapStatuses = new mutable.HashMap[Int, Array[Byte]]
  val mapStatuses: Map[Int, Array[MapStatus]] =
    new ConcurrentHashMap[Int, Array[MapStatus]]().asScala
 // val shuffleServiceMapOutpoutTrackerRpcEnv: RpcEnv = RpcEnv.create("shuffleServiceMapOutpoutTracker", "localhost", 7799, conf, securityMgr)
 ...
 // val driverPort: Int = conf.getInt(config.DRIVER_PORT.key, 7077)
  /*mapOutputTracker.trackerEndpoint = rpcEnv.setupEndpointRef(RpcAddress(driverHost, driverPort), "spark-driver")*/
  //mapOutputTracker.trackerEndpoint = RpcUtils.makeDriverRef(MapOutputTracker.ENDPOINT_NAME, conf, rpcEnv)
 // val driverAddress: RpcAddress = RpcAddress(driverHost, driverPort)
  //---val driverEndpoint: RpcEndpointRef = shuffleServiceMapOutpoutTrackerRpcEnv.setupEndpointRef(driverAddress, MapOutputTracker.ENDPOINT_NAME)
  ...
  private val fetchingLock = new KeyLock[Int]
   ...
   private[spark] def getMapSizesByExecutorId(
                                        shuffleId: Int,
                                        serializedMapStatus: Array[Byte],
                                        startMapIndex: Int,
                                        endMapIndex: Int,
                                        startPartition: Int,
                                        endPartition: Int): Iterator[(BlockManagerId, Seq[(BlockId, Long, Int)])] = {
     logDebug(s"Fetching outputs for shuffle $shuffleId")
     val statuses = getStatuses(shuffleId, serializedMapStatus, conf)
     //val statuses = MapOutputTracker.deserializeMapStatusesForShuffleService(serializedMapStatus, conf) //deserializeMapStatuses(serializedMapStatus, conf)
     //logDebug(s"CHECK statuses: $statuses")
     try {
       val actualEndMapIndex = if (endMapIndex == Int.MaxValue) statuses.length else endMapIndex
       logDebug(s"Convert map statuses for shuffle $shuffleId, " +
         s"mappers $startMapIndex-$actualEndMapIndex, partitions $startPartition-$endPartition")
   ...
   * (It would be nice to remove this restriction in the future.)
   */
  private def getStatuses(shuffleId: Int, serializedMapStatus: Array[Byte], conf: SparkConf): Array[MapStatus] = {
    val statuses = mapStatuses.get(shuffleId).orNull
    logTrace(s"mapStatuses size: ${mapStatuses.size}")
    if (statuses == null) {
      ...
      var fetchedStatuses = mapStatuses.get(shuffleId).orNull
      if (fetchedStatuses == null) {
        //val fetchedBytes = serializedMapStatus //askTracker[Array[Byte]](GetMapOutputStatuses(shuffleId))
        fetchedStatuses = MapOutputTracker.deserializeMapStatusesForShuffleService(serializedMapStatus, conf) //MapOutputTracker.deserializeMapStatuses(fetchedBytes, conf)
        logDebug(s"Current fetched mapStatuses length: ${fetchedStatuses.length}")
        logTrace("Got the output locations")
        mapStatuses.put(shuffleId, fetchedStatuses)
        }
      logDebug(s"Fetching map output statuses for shuffle $shuffleId took " +
        s"${TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - startTimeNs)} ms")
      logTrace(s"MapStatuses: $fetchedStatuses")
      fetchedStatuses
      //}
    } else {
      statuses
   ...
   *         Note that zero-sized blocks are excluded in the result.
   */
  override def getMapSizesByExecutorId(shuffleId: Int, startMapIndex: Int, endMapIndex: Int, startPartition: Int, endPartition: Int): Iterator[(BlockManagerId, Seq[(BlockId, Long, Int)])] = ???
}
...
private[spark] object MapOutputTracker extends Logging {
  ...
  // Serialize an array of map output locations into an efficient byte format so that we can send
  // it to the CHERRY shuffle service. We do this by compressing the serialized bytes using Zstd. They will
  // generally be pretty compressible because many map outputs will be on the same hostname.
  def serializeMapStatusesForShuffleService(
    statuses: Array[MapStatus],
    conf: SparkConf): Array[Byte] = {
    // Using `org.apache.commons.io.output.ByteArrayOutputStream` instead of the standard one
    // This implementation doesn't reallocate the whole memory block but allocates
  ...
  // Opposite of serializeMapStatuses.
  def deserializeMapStatusesForShuffleService(bytes: Array[Byte], conf: SparkConf): Array[MapStatus] = {
    assert (bytes.length > 0)
    ...
    deserializeObject(bytes, 0, bytes.length).asInstanceOf[Array[MapStatus]]
  }
