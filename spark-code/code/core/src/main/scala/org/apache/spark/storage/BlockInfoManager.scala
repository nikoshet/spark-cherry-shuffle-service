...
private[storage] class BlockInfoManager extends Logging {
  ...
  /**
   * ADDED
   * Lock a shuffle block for reading and return its metadata.
   */
  def lockForShuffleReading(
                             blockId: BlockId,
                             blocking: Boolean = true): BlockInfo = synchronized {
    logTrace(s"Task $currentTaskAttemptId trying to acquire read lock for $blockId")
    do {
      infos.get(blockId) match {
        case Some(info) =>
    ...
    //val info = BlockInfo.getClass[_]
    //info
    BlockInfo.asInstanceOf
  }
