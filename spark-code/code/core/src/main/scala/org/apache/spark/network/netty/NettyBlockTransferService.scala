...
private[spark] class NettyBlockTransferService(
  ...
  override def uploadBlock(
    ...
    logger.trace(s"metadata: ${metadata.toString}")
    logger.trace(s"blockdata: ${blockData.toString}")
    ...
    val callback = new RpcResponseCallback {
      override def onSuccess(response: ByteBuffer): Unit = {
        ...
        if (blockFilePath.contains(".data")){
          logger.info(s"Finished UploadBlockSync of Data in ${System.currentTimeMillis - startTime} ms")
        }
        else if (blockFilePath.contains(".index")){
          logger.info(s"Finished UploadBlockSync of Index in ${System.currentTimeMillis - startTime} ms")
        }
        startTime = 0
        result.success((): Unit)
      }
    ...
    if (asStream) {
      ...
      val streamHeader = new UploadBlockStream(blockFilePath, metadata).toByteBuffer
      client.uploadStream(new NioManagedBuffer(streamHeader), blockData, callback)
      //client.send(blockData.nioByteBuffer())
      //val array = JavaUtils.bufferToArray(blockData.nioByteBuffer())
      //client.send(new UploadBlock(appId, execId, blockId.name, metadata, array).toByteBuffer)
    } else {
      // Convert or copy nio buffer into array in order to serialize it.
      val array = JavaUtils.bufferToArray(blockData.nioByteBuffer())
      /*client.sendRpc(new UploadBlock(appId, execId, blockId.name, metadata, array).toByteBuffer,
        callback)*/
      client.sendRpc(new UploadBlock(appId, execId, blockFilePath, metadata, array).toByteBuffer,
        callback)
      //client.send(blockData.nioByteBuffer())
      //client.send(new UploadBlock(appId, execId, blockId.name, metadata, array).toByteBuffer)
    }
