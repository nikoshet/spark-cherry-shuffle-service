...
import org.apache.spark.scheduler.cluster.CoarseGrainedClusterMessages.{GetCherryNodesInfo, InformDriverSchedulerAboutCherryNodes, RegisterExecutor, RegisterNewCherryNode}
...
private[spark] class BlockManager(
  ...
  // An Array of CHERRY shuffle service addresses that the executors
  // will use to push and fetch intermediate shuffle data.
  private[spark] var cherryShuffleServerIdArray: Array[BlockManagerId] = Array()//= _ // = new Array(0)
  private[spark] var cherryHostArray: Array[String] = Array()
  private[spark] var tempSelectedShuffleServerId: BlockManagerId = _
  private lazy val cherryNodeId = new AtomicInteger(0)
  private[spark] def getNextCherryId: Int = cherryNodeId.getAndIncrement()

  // Client to read other executors' blocks. This is either an external service, or just the
  // standard BlockTransferService to directly connect to other Executors.
  // removed private[spark]
  // private[spark] blockStoreClient = externalBlockStoreClient.getOrElse(blockTransferService)
  val blockStoreClient: BlockStoreClient = externalBlockStoreClient.getOrElse(blockTransferService)
  ...
  def initialize(appId: String): Unit = {
    ...
    /*shuffleServerId = if (externalShuffleServiceEnabled && conf.get(config.SHUFFLE_SERVICE_AVAILABILITY) != "cherry") {
      logInfo(s"external shuffle service port = $externalShuffleServicePort")
      BlockManagerId(executorId, blockTransferService.hostName, externalShuffleServicePort)
    } else if (externalShuffleServiceEnabled && conf.get(config.SHUFFLE_SERVICE_AVAILABILITY) == "cherry") {
      val remote_service_host = conf.get(config.SHUFFLE_SERVICE_HOST)
      logInfo(s"REMOTE external shuffle service host = $remote_service_host")
      logInfo(s"REMOTE external shuffle service port = $externalShuffleServicePort")
      BlockManagerId(executorId, remote_service_host, externalShuffleServicePort)
    } else{
      blockManagerId
    }*/

    shuffleServerId = if (externalShuffleServiceEnabled){
      if (conf.get(config.SHUFFLE_SERVICE_AVAILABILITY) != "cherry"){
        logInfo(s"external shuffle service port = $externalShuffleServicePort")
        BlockManagerId(executorId, blockTransferService.hostName, externalShuffleServicePort)
      }
      else{ // if (conf.get(config.SHUFFLE_SERVICE_AVAILABILITY) == "cherry") {
        if (conf.get(config.DISTRIBUTED_CHERRY_ENABLED)){
          logInfo(s"Distributed CHERRY is enabled.")

          val metadataHost = conf.get(config.METADATA_SERVICE_HOST)
          val metadataPort = conf.get(config.METADATA_SERVICE_PORT)
          // Get CHERRY node info
          val metadataEndpoint = rpcEnv.setupEndpointRef(RpcAddress(metadataHost, metadataPort), "MetadataServiceEndpoint")
          val serCherryNodeInfoMap = metadataEndpoint.askSync[Array[Byte]](GetCherryNodesInfo()) //.asInstanceOf[Array[Byte]]
          val closureSerializer = new JavaSerializer(conf)
          val ser: SerializerInstance = closureSerializer.newInstance()

          val cherryNodeInfoMap = ser.deserialize[mutable.HashMap[Int, Array[_]]](
            ByteBuffer.wrap(serCherryNodeInfoMap), Thread.currentThread.getContextClassLoader)
          cherryNodeInfoMap.foreach { case (key, values) => logTrace("key " + key + " -> " + values.mkString(" : "))
          }
          // Add CHERRY node info in executor's persistent array
          //...
          for ((key, values) <- cherryNodeInfoMap) {
            /*sparkWorkerInfoMap.foreach {
              case (key, values) =>*/
            val keyId = key.toString // .asInstanceOf[String]
            val cherryHost = values.apply(0).asInstanceOf[String]
            val cherryPort = values.apply(1).asInstanceOf[Int]
            tempSelectedShuffleServerId = BlockManagerId(executorId, cherryHost, cherryPort) //keyId or executorId
            // Add in array
            cherryShuffleServerIdArray = cherryShuffleServerIdArray :+ tempSelectedShuffleServerId
            logInfo(s"cherryShuffleServerIdArray size: ${cherryShuffleServerIdArray.length}")
          }

          if (blockManagerId.isDriver) {
            cherryNodeInfoMap.foreach { case (key, values) =>
              logInfo(s"Value to add in cherryHostArray: ${values.apply(0).asInstanceOf[String]}")
              cherryHostArray = cherryHostArray :+ values.apply(0).asInstanceOf[String]
            }
            logInfo(s"CherryHostArray to send to driverScheduler: ${cherryHostArray}")
            val driverUrl = rpcEnv.setupEndpointRef(RpcAddress(conf.get(config.DRIVER_HOST_ADDRESS),
              conf.get(config.DRIVER_PORT)), "CoarseGrainedScheduler")
              driverUrl.send(InformDriverSchedulerAboutCherryNodes(cherryHostArray))
          }

          //val remote_service_host = conf.get(config.SHUFFLE_SERVICE_HOST)
          //logInfo(s"CHERRY shuffle service host = $remote_service_host")
          //logInfo(s"CHERRY shuffle service port = $externalShuffleServicePort")
          //BlockManagerId(executorId, remote_service_host, externalShuffleServicePort)
          blockManagerId
        }
        else{
  ...
  /**
   * From the available Cherry nodes, get a cherry
   * node Id to send shuffle data based on modulo operation
  **/
  def getCherryShuffleServiceNodeId: BlockManagerId = {
    val availableCherryNodes = cherryShuffleServerIdArray.length
    tempSelectedShuffleServerId = cherryShuffleServerIdArray.apply((availableCherryNodes + getNextCherryId) % availableCherryNodes)
    logDebug(s"Selected temp CherryShuffleServerId: ${tempSelectedShuffleServerId.host} : ${tempSelectedShuffleServerId.port}")
    tempSelectedShuffleServerId
  }
  ...
  private def registerWithExternalShuffleServer(): Unit = {
    ...
    logDebug(s"Shuffle Config: $shuffleConfig")
    // DEBUG BlockManager: Shuffle Config: ExecutorShuffleInfo[localDirs=[/home/ubuntu/spark-tmp/spark-f8b6f16a-65b0-4828-a3dd-381b289f59a4/executor-d63274e2-8064-4fcb-877f-91349362e483/blockmgr-aa3412c8-7427-4739-a743-7e285fcc8640],
    ...
    for (i <- 1 to MAX_ATTEMPTS) {
      try {
        ...
        // Register in all CHERRY nodes available
        if (conf.get(config.DISTRIBUTED_CHERRY_ENABLED)){
          cherryShuffleServerIdArray.foreach({ cherryShuffleServiceId =>
            blockStoreClient.asInstanceOf[ExternalBlockStoreClient].registerWithShuffleServer(
              cherryShuffleServiceId.host, cherryShuffleServiceId.port, shuffleServerId.executorId, shuffleConfig) //cherryShuffleServiceId.executorId, shuffleConfig)
          })
        }
        else {
          blockStoreClient.asInstanceOf[ExternalBlockStoreClient].registerWithShuffleServer(
  ...
  /**
   * Only used when new Cherry node is added
   * in cluster and registered in Metadata
   * Service while a workload is being executed.
   **/
  def registerWithNewCherryShuffleSerice(cherryHost: String, cherryPort: Int, executorId: String): Unit = {
    logInfo("Registering executor with new Cherry Shuffle Service")
    val shuffleConfig = new ExecutorShuffleInfo(
      diskBlockManager.localDirsString,
      diskBlockManager.subDirsPerLocalDir,
      shuffleManager.getClass.getName)
    ...
    val MAX_ATTEMPTS = conf.get(config.SHUFFLE_REGISTRATION_MAX_ATTEMPTS)
    val SLEEP_TIME_SECS = 2

    for (i <- 1 to MAX_ATTEMPTS) {
      try {
        // Synchronous and will throw an exception if we cannot connect.
        blockStoreClient.asInstanceOf[ExternalBlockStoreClient].registerWithShuffleServer(
          cherryHost, cherryPort, executorId, shuffleConfig)
        return
      } catch {
        case e: Exception if i < MAX_ATTEMPTS =>
          logError(s"Failed to connect to external shuffle server, will retry ${MAX_ATTEMPTS - i}"
  ...
  private def doPut[T](
    ...
    logTrace(s"---doPut func, blockId: $blockId")
    require(blockId != null, "BlockId is null")
    ...
    while(numFailures <= maxReplicationFailureCount &&
      ...
      try {
        ...
        blockTransferService.uploadBlockSync(
          ...
          peer.executorId,
          blockId,
          blockId.toString, // must return datafile
          buffer,
          tLevel,
          classTag)
