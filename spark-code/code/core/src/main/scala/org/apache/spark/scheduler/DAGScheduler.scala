...
import org.apache.spark.internal.config.SHUFFLE_MAPOUTPUT_MIN_SIZE_FOR_BROADCAST
import org.apache.spark.internal.config.Tests.TEST_NO_STAGE_RETRY
import org.apache.spark.network.util.JavaUtils
import org.apache.spark.partial.{ApproximateActionListener, ApproximateEvaluator, PartialResult}
import org.apache.spark.rdd.{RDD, RDDCheckpointData, UnionRDD}
...
import org.apache.spark.scheduler.cluster.CoarseGrainedClusterMessages.{BroadCastTaskBinary, LaunchTask, SerMapStatusForResultStage, SerMapStatusForShuffleMapStage, SerMapStatusForShuffleMapStageUnion}
...
private[spark] class DAGScheduler(
  ...
  private[spark] val waitingStages = new HashSet[Stage] //[scheduler]

  // Stages we are running right now
  private[spark] val runningStages = new HashSet[Stage] //[scheduler]
  ...
  private[spark] val cherryDataMap = new HashMap[String, RpcEndpointRef]
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int): Unit = {
    ...
    try {
      ...
      RDDCheckpointData.synchronized {
        ...
        /*** ***/
        // I need the send new serMapStatus info for each stage of the DAG except the stages
        // that have no parent stages (they create or read external data) => no shuffle data yet
        //for testing:
        if (stage.parents.isEmpty){
          logDebug("Current stage is a ShuffleMapStage with no parent stages.")
        }
        //if (sc.conf.get(config.LOOK_AHEAD_CACHING_ENABLED) && stage.parents.nonEmpty) {
        if (sc.conf.get(config.SHUFFLE_SERVICE_ENABLED) && sc.conf.get(config.SHUFFLE_SERVICE_AVAILABILITY) == "cherry" && sc.conf.get(config.LOOK_AHEAD_CACHING_ENABLED) && stage.parents.nonEmpty) {

          if (sc.conf.get(config.DISTRIBUTED_CHERRY_ENABLED)) {

            /*new Thread("DAGScheduler-send-message-to-Cherry-nodes") {
              override def run(): Unit = {*/


            cherryDataMap.foreach{ case (key, cherryNodeEndpoint) =>
              sendSerMapStatusesToCherry(partitions, stage, cherryNodeEndpoint)
            }
              /*env.blockManager.cherryShuffleServerIdArray.foreach({ cherryShuffleServerId =>
                logDebug(s"Sending SerMapStatus info to Cherry node Id: $cherryShuffleServerId")
                val cherryRpcAddress = RpcAddress(cherryShuffleServerId.host, sc.conf.get(config.LOOK_AHEAD_CACHING_PORT))
                sendSerMapStatusesToCherry(partitions, stage, cherryRpcAddress)
              })*/
          /*}
            }.start()*/
            ...
            val cherryRpcAddress = RpcAddress(sc.conf.get(config.SHUFFLE_SERVICE_HOST), sc.conf.get(config.LOOK_AHEAD_CACHING_PORT))
            val cherryEndpoint = sc.env.rpcEnv.setupEndpointRef(cherryRpcAddress, "RemoteShuffleServiceEndpoint")
            sendSerMapStatusesToCherry(partitions, stage, cherryEndpoint)
          }
  ...
  /**
   * Send to the CHERRY shuffle service the serialized map statuses of the
   * parent stage that this stage is dependent on.
   *
   * For ShuffleMapStage/ResultStage we send:
   * - stage.id : the id of this stage.
   * - "ShuffleMapStage" / "ResultStage" : a string that works as an identifier of stage type.
   * - stage.rdd.getNumPartitions : number of partitions for each task.
   * - shuffleId : the shuffle Id of the parent stage.
   * - serMapStatus : the serialized Map Statuses. The serialized bytes are compressed using Zstd.
   */
  private def sendSerMapStatusesToCherry(partitions: Array[Partition], stage: Stage, cherryEndpoint: RpcEndpointRef): Unit = {


    logDebug(s"Number of parent stages for this stage: ${stage.parents.size}")
    logInfo(s"Sending Broadcast value to shuffle service")
    //val rssRpcAddress = cherryRpcAddress //RpcAddress(sc.conf.get(config.SHUFFLE_SERVICE_HOST),sc.conf.get(config.LOOK_AHEAD_CACHING_PORT))
    //val cherryEndpoint = sc.env.rpcEnv.setupEndpointRef(cherryRpcAddress, "RemoteShuffleServiceEndpoint")

    logInfo(s"Stage RDD: ${stage.rdd}")
    stage match {
      case stage: ShuffleMapStage =>
        logDebug(s"ShuffleMapStage, shuffleDep: ${stage.shuffleDep}")

        if (stage.parents.length > 1){
          logDebug(s"Multiple stage parents of number: ${stage.parents.length}")

          var shuffleIdArrayBuffer =  ArrayBuffer[Int]() //Array[Int]
          var serMapStatusArrayBuffer =  ArrayBuffer[Array[Byte]]()  //Array[Array[Byte]]

          stage.parents.foreach { parent =>

            // Get each shuffleId of the shuffle each stage is part of
            val dep = parent.asInstanceOf[ShuffleMapStage].shuffleDep
            val shuffleId = dep.shuffleHandle.shuffleId

            /*----*/
            /*val dep_0 = stage.rdd.dependencies.head //.asInstanceOf[ShuffleDependency[_, _, _]]
            val shuffleId_0 = dep_0.shuffleHandle.shuffleId
            logInfo(s"This stage is dependent on the shuffleId TEST: $shuffleId_0")
            //
            val dep1 = stage.rdd.dependencies.head
            val shuffleId1 = stage.shuffleDep.shuffleId
            logInfo(s"This stage is dependent on the shuffleId TEST: $shuffleId1")
            //
            val dep2 = stage.shuffleDep
            val shuffleId2 = dep2.shuffleHandle.shuffleId
            logInfo(s"This stage is dependent on the shuffleId TEST: $shuffleId2")
            //
            val dep3 = stage.shuffleDep
            var shuffleId3 = 0
            dep3 match {
              case value: ShuffleDependency[_, _, _] =>
                shuffleId3 = value.shuffleId
              case _ =>
            }
            logInfo(s"This stage is dependent on the shuffleId TEST: $shuffleId3")*/
            /*----*/

            // Get the Array of map statuses of the specific shuffleId.
            // Includes the block manager address that the
            // task has shuffle files stored on as well as the sizes of outputs for each reducer, for passing
            // on to the reduce tasks.
            // This Array is completely filled only after the parent stage(s) has completely finished.

            //val minSizeForBroadcast = sc.conf.get(SHUFFLE_MAPOUTPUT_MIN_SIZE_FOR_BROADCAST).toInt //Integer.MAX_VALUE//
            /*val serMapStatus = shuffleStatus.serializedMapStatus(sc.env.broadcastManager, sc.isLocal, minSizeForBroadcast,
              sc.conf)*/
            //logDebug(s"serializedMapStatus INFO FROM DAGSCHEDULER to send: ${sc.env.broadcastManager} , ${sc.isLocal} , ${minSizeForBroadcast} , ${sc.conf}")
            //logDebug(s"serializedMapStatus FROM DAGSCHEDULER to send: ${serMapStatus}")
            val shuffleStatus = mapOutputTracker.shuffleStatuses.get(shuffleId).head
            val serMapStatus = shuffleStatus.serializedMapStatusForShuffleService(sc.conf)
            logDebug(s"Current shuffleStatus.mapStatuses length: ${shuffleStatus.mapStatuses.length}")
            shuffleIdArrayBuffer = shuffleIdArrayBuffer :+ shuffleId
            serMapStatusArrayBuffer =  serMapStatusArrayBuffer :+ serMapStatus

            //testing
            val numRdds = parent.rdd.dependencies.length
            logDebug(s"Num of dep rdds: ${numRdds}")
            logDebug(s"Partitions length of rdd ${parent.rdd.partitions.length}")
            logDebug(s"Class name of rdd ${parent.rdd.getClass.getName}")
            /*stage.rdd match {
              case org.apache.spark.rdd.UnionRDD[_] =>
                logInfo("UnionRDD RDD")
              case CoGroupedRDD =>
                logInfo("CoGroupedRDD RDD")
              case UnionPartition =>
                logInfo("UnionPartition RDD")
            }*/
          }

          val shuffleIdArray = shuffleIdArrayBuffer.toArray
          val serMapStatusArray = serMapStatusArrayBuffer.toArray
          logInfo(s"This stage is dependent on the shuffleId(s): ${shuffleIdArray.mkString(", ")} ,  and number of serMapStatuses: ${serMapStatusArray.length}")
          cherryEndpoint.send(SerMapStatusForShuffleMapStageUnion(JavaUtils.bufferToArray(
            closureSerializer.serialize(stage.id, "ShuffleMapStageUnion", stage.rdd.getNumPartitions, shuffleIdArray, partitions)), serMapStatusArray))
        }
        else{
          val dep = stage.parents.head.asInstanceOf[ShuffleMapStage].shuffleDep
          val shuffleId = dep.shuffleHandle.shuffleId
          ...
          val numRdds = dep.rdd.dependencies.length
          logDebug(s"Num of dep rdds: ${numRdds}")
          logDebug(s"Partitions length of rdd ${dep.rdd.partitions.length}")
          logDebug(s"Class name of rdd ${dep.rdd.getClass.getName}")
          logInfo(s"This stage is dependent on the shuffleId: $shuffleId")
          cherryEndpoint.send(SerMapStatusForShuffleMapStage(JavaUtils.bufferToArray(
            closureSerializer.serialize(stage.id, "ShuffleMapStage", stage.rdd.getNumPartitions, shuffleId)), serMapStatus))
        }

        //logDebug(s"serializedMapStatus NEW FROM DAGSCHEDULER to send: ${shuffleStatus.serializedMapStatusForShuffleService(sc.conf)}")
        //logDebug(s"serializedMapStatus NEW FROM DAGSCHEDULER mapStatuses to send: ${shuffleStatus.mapStatuses}")

        /*cherryEndpoint.send(BrValueForShuffleMapStage(JavaUtils.bufferToArray(
          closureSerializer.serialize(stage.id, "ShuffleMapStage", stage.rdd, stage.shuffleDep))))*/
      case stage: ResultStage =>
        logDebug(s"ResultStage, func: ${stage.func}")

        // Get each shuffleId of the shuffle each stage is part of
        /*val dep = stage.rdd.dependencies.head
        var shuffleId = 0
        dep match {
          case value: ShuffleDependency[_, _, _] =>
        ...
        // task has shuffle files stored on as well as the sizes of outputs for each reducer, for passing
        // on to the reduce tasks.
        // This Array is completely filled only after the parent stage has completely finished.
        ...
        // to avoid "java.util.NoSuchElementException: head of empty list" from TPCDS Benchmark
        if (mapOutputTracker.shuffleStatuses.contains(shuffleId)){
          val shuffleStatus = mapOutputTracker.shuffleStatuses.get(shuffleId).head
          val serMapStatus = shuffleStatus.serializedMapStatusForShuffleService(sc.conf)
          //logDebug(s"serializedMapStatus NEW FROM DAGSCHEDULER to send: ${shuffleStatus.serializedMapStatusForShuffleService(sc.conf)}")
          //logDebug(s"serializedMapStatus NEW FROM DAGSCHEDULER mapStatuses to send: ${shuffleStatus.mapStatuses}")
          cherryEndpoint.send(SerMapStatusForResultStage(JavaUtils.bufferToArray( //SerMapStatusForResultStage
            closureSerializer.serialize(stage.id, "ResultStage", stage.rdd.getNumPartitions, shuffleId)), serMapStatus))
          //rssEndpoint.send(BroadCastTaskBinaryForResultStage(stage.id, "ResultStage", stage.rdd, stage.func))
        }
      //}
