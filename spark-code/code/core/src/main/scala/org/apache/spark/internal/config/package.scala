...
package object config {
  ...
  private[spark] val SHUFFLE_SERVICE_HOST =
    ConfigBuilder("spark.shuffle.service.host").version("3.0.1").stringConf.createWithDefaultString("localhost")

  private[spark] val SHUFFLE_SERVICE_AVAILABILITY =
    ConfigBuilder("spark.shuffle.service.availability").version("3.0.1").stringConf.createWithDefaultString("local") // or "cherry"

  private[spark] val LOOK_AHEAD_CACHING_ENABLED =
    ConfigBuilder("spark.shuffle.service.look.ahead.caching.enabled")
      .version("3.0.1")
      .booleanConf
      .createWithDefault(false)

  private[spark] val LOOK_AHEAD_CACHING_SIZE =
    ConfigBuilder("spark.shuffle.service.look.ahead.caching.size")
      .version("3.0.1")
      .intConf.createWithDefault(0) //number of (K,V) pairs

  private[spark] val LOOK_AHEAD_CACHING_PORT =
    ConfigBuilder("spark.shuffle.service.look.ahead.caching.port")
      .version("3.0.1")
      .intConf.createWithDefault(7788)

  private[spark] val DISTRIBUTED_CHERRY_ENABLED =
    ConfigBuilder("spark.distributed.cherry.enabled")
      .version("3.0.1")
      .booleanConf
      .createWithDefault(false)

  private[spark] val METADATA_SERVICE_PORT =
    ConfigBuilder("spark.metadata.service.port").version("1.2.0").intConf.createWithDefault(5555)

  private[spark] val METADATA_SERVICE_HOST =
    ConfigBuilder("spark.metadata.service.host").version("3.0.1").stringConf.createWithDefaultString("localhost")
