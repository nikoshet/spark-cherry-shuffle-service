      ...
      stage.jobs.foreach { job =>
        stage.status match {
          case v1.StageStatus.COMPLETE =>
            ...
            //
            // Log shuffle read/write time/size metrics
            val totalShuffleWriteTime = (stage.metrics.shuffleWriteMetrics.writeTime/ 1e6).toLong
            val meanShuffleWriteTime = totalShuffleWriteTime / stage.info.numTasks
            val shuffleBytesWritten = stage.metrics.shuffleWriteMetrics.bytesWritten
            logInfo(s"Stage ${stage.info.stageId} Total Shuffle Write Time: ${UIUtils.formatDuration(totalShuffleWriteTime)}")
            logInfo(s"Stage ${stage.info.stageId} Mean Shuffle Write Time: ${UIUtils.formatDuration(meanShuffleWriteTime)}")
            logInfo(s"Stage ${stage.info.stageId} Total Shuffle Write Size: ${Utils.bytesToString(shuffleBytesWritten)}")

            val deserializationTime = stage.metrics.executorDeserializeTime
            val gcTime = stage.metrics.jvmGcTime
            logInfo(s"Stage ${stage.info.stageId} Total Deserialization Time: ${UIUtils.formatDuration(deserializationTime)}")
            logInfo(s"Stage ${stage.info.stageId} Total GC Time: ${UIUtils.formatDuration(gcTime)}")

            val totalShuffleReadTime = stage.metrics.shuffleReadMetrics.fetchWaitTime
            val meanShuffleReadTime = totalShuffleReadTime / stage.info.numTasks
            val shuffleBytesRead = stage.metrics.shuffleReadMetrics.localBytesRead + stage.metrics.shuffleReadMetrics.remoteBytesRead
            logInfo(s"Stage ${stage.info.stageId} Total Shuffle Read Time: ${UIUtils.formatDuration(totalShuffleReadTime)}")
            //logDebug(s"Stage ${stage.info.stageId} Local Shuffle Read Time: ${UIUtils.formatDuration(stage.metrics.shuffleReadMetrics.localBytesRead)}")
            logInfo(s"Stage ${stage.info.stageId} Mean Shuffle Read Time: ${UIUtils.formatDuration(meanShuffleReadTime)}")
            logInfo(s"Stage ${stage.info.stageId} Total Shuffle Read Size: ${Utils.bytesToString(shuffleBytesRead)}")

            val executorRunTime = stage.metrics.executorRunTime
            logInfo(s"Stage ${stage.info.stageId} Total Time Across All Tasks: ${UIUtils.formatDuration(executorRunTime)}")

            val clock: Clock = new SystemClock()
            val totalTime = stage.info.submissionTime match {
              case Some(t) => "%.03f".format((clock.getTimeMillis() - t) / 1000.0)
              case _ => "Unknown"
            }
            logInfo(s"Stage ${stage.info.stageId} Total Time: ${totalTime} s")
            //
