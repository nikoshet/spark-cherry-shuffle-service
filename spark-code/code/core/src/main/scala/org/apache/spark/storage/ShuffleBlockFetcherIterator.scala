...
final class ShuffleBlockFetcherIterator(
  ...
  private[this] def sendRequest(req: FetchRequest): Unit = {
    ...
    logTrace(s"---- infoMap: $infoMap, blockIds: $blockIds, address: $address")
    //---- infoMap: Map(shuffle_0_0_1929 -> (129,0)), blockIds: List(shuffle_0_0_1929), address: BlockManagerId(0, 10.0.1.169, 7337, None)
    logTrace(s"---- Req Blocks: ${req.blocks}")
    //---- Req Blocks: List(FetchBlockInfo(shuffle_0_85_68,171,0))
    ...
    if (req.size > maxReqSizeShuffleToMem) {
      logDebug("DOWNLOADFILEMANAGER=THIS")
      shuffleClient.fetchBlocks(address.host, address.port, address.executorId, blockIds.toArray,
        blockFetchingListener, this)
    } else {
      logDebug("DOWNLOADFILEMANAGER=NULL")
      shuffleClient.fetchBlocks(address.host, address.port, address.executorId, blockIds.toArray,
