...
import com.codahale.metrics.{Counter, Gauge, Meter, Metric}
...
private[deploy] object DeployMessages {
  ...
  //  ShuffleServiceWebUI to Shuffle Service

  case object RequestShuffleServiceState

  // Shuffle Service to ShuffleServiceWebUI
  // ---- TO ADD: Metrics Of External Shuffle Service
  // ---- Currently added metrics: ExternalBlockHandler
  case class ShuffleServiceStateResponse(host: String, port: Int, blockTransferRateBytes: String,
                                         registeredExecutorsSize: String, numActiveConnections: String,
                                         numCaughtExceptions: String) {
    Utils.checkHost(host)
    //config.SHUFFLE_SERVICE_HOST
    assert (port > 0)
