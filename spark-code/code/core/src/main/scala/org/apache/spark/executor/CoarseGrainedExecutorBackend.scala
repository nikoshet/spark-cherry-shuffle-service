...
private[spark] class CoarseGrainedExecutorBackend(
  ...
  override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
    ...
    case NewCherryNodeRegistered(msg) =>
      context.reply(true)
      val (cherryIp, cherryPort) = ser.deserialize[(String, Int)](
        ByteBuffer.wrap(msg), Thread.currentThread.getContextClassLoader)
      logInfo(s"NewCherryNodeRegistered message arrived. IP: ${cherryIp}, PORT: ${cherryPort}")
      // Register in new Cherry Node that is added in cluster
      env.blockManager.registerWithNewCherryShuffleSerice(cherryIp, cherryPort, executorId)
      // Add in array
      env.blockManager.cherryShuffleServerIdArray = env.blockManager.cherryShuffleServerIdArray :+ BlockManagerId(executorId, cherryIp, cherryPort) //keyId or executorId
