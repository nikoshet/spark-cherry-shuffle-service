...
import org.apache.spark.scheduler.cluster.CoarseGrainedClusterMessages.DeregisterExec
...
private[spark] class Executor(
  ...
  def stop(): Unit = {
    if (conf.get(SHUFFLE_SERVICE_ENABLED) && conf.get(SHUFFLE_SERVICE_AVAILABILITY) == "cherry"){
      // Degister in all CHERRY nodes available
      if (conf.get(DISTRIBUTED_CHERRY_ENABLED)){
        logInfo("Deregistering from Distributed Cherry nodes")
        env.blockManager.cherryShuffleServerIdArray.foreach({ cherryShuffleServiceId =>
          env.blockManager.blockStoreClient.asInstanceOf[ExternalBlockStoreClient].deregisterFromShuffleServer(cherryShuffleServiceId.host, cherryShuffleServiceId.port, executorId)
        })
        logInfo("Deregistering from Metadata Service")
        val metadataHost = conf.get(config.METADATA_SERVICE_HOST)
        val metadataPort = conf.get(config.METADATA_SERVICE_PORT)
        val metadataEndpoint = env.rpcEnv.setupEndpointRef(RpcAddress(metadataHost, metadataPort), "MetadataServiceEndpoint")
        metadataEndpoint.askSync[Boolean](DeregisterExec())
      }
      else {
        logInfo("Deregistering from Shuffle Server")
        env.blockManager.blockStoreClient.asInstanceOf[ExternalBlockStoreClient].deregisterFromShuffleServer(conf.get(SHUFFLE_SERVICE_HOST), conf.get(SHUFFLE_SERVICE_PORT), executorId)
      }
    }
