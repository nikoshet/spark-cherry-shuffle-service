      ...
      <svg>
        {
          val legendPairs = List(("scheduler-delay-proportion", "Scheduler Delay"),
            ...
            ("shuffle-read-records-proportion", "Shuffle Records Read"),
            ("shuffle-read-bytes-proportion", "Shuffle Bytes Read"),
            ("shuffle-write-records-proportion", "Shuffle Records Written"),
            ("shuffle-write-bytes-proportion", "Shuffle Bytes Written")*/
  ...
  def makeTimeline(
    ...
    val executorsArrayStr =
      tasks.sortBy(-_.launchTime.getTime()).take(MAX_TIMELINE_TASKS).map { taskInfo =>
        ...
        // added //
        val shuffleRecordsRead =
          metricsOpt.map(_.shuffleReadMetrics.recordsRead).getOrElse(0L)
        val shuffleBytesRead =
          metricsOpt.map(_.shuffleReadMetrics.localBytesRead).getOrElse(0L) + metricsOpt.map(_.shuffleReadMetrics.remoteBytesRead).getOrElse(0L)
        val shuffleRecordsWritten =
          metricsOpt.map(_.shuffleWriteMetrics.recordsWritten).getOrElse(0L)
        val shuffleBytesWritten =
          metricsOpt.map(_.shuffleWriteMetrics.bytesWritten).getOrElse(0L)
        // \added //
          ...
          s"""
             |{
               ...
               |'content': '<div class="task-assignment-timeline-content"
                 ...
                 |<br>Shuffle Records Read: ${shuffleRecordsRead}
                 |<br>Shuffle Bytes Read: ${shuffleBytesRead}
                 |<br>Shuffle Records Written: ${shuffleRecordsWritten}
                 |<br>Shuffle Bytes Written: ${shuffleBytesWritten}">
                 |$svgTag',
