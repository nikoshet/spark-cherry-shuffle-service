...
class CoarseGrainedSchedulerBackend(scheduler: TaskSchedulerImpl, val rpcEnv: RpcEnv)
  ...
  class DriverEndpoint extends IsolatedRpcEndpoint with Logging {
    ...
    override def receive: PartialFunction[Any, Unit] = {
      ...
      case InformDriverSchedulerAboutCherryNodes(cherryArray) =>
        logInfo(s"Got list of available cherry nodes of size: ${cherryArray.length}")
        cherryArray.foreach { host =>
          val cherryEndpoint = rpcEnv.setupEndpointRef(RpcAddress(host, conf.get(config.LOOK_AHEAD_CACHING_PORT)), "RemoteShuffleServiceEndpoint")
          scheduler.sc.dagScheduler.cherryDataMap.put(host, cherryEndpoint)
        }
    ...
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]): Unit = {
      for (task <- tasks.flatten) {
        ...
        if (serializedTask.limit() >= maxRpcMessageSize) {
          ...
          /*val informCherryThread = new Thread() {
            override def run(): Unit = {*/
              // Send serializedTaskMessage to Remote Shuffle Service
              if (conf.get(config.SHUFFLE_SERVICE_ENABLED) && conf.get(config.SHUFFLE_SERVICE_AVAILABILITY) == "cherry" && conf.get(config.LOOK_AHEAD_CACHING_ENABLED)) {

                //val t1 = System.nanoTime

                // I need the send new launch task info for each stage of the DAG except the stages
                ...
                val descrTask = ser.deserialize[Task[Any]](task.serializedTask, Thread.currentThread.getContextClassLoader)

                //val duration1 = (System.nanoTime - t1) / 1e9d
                //logInfo(s"Duration of message prep1: $duration1")
                //val t2 = System.nanoTime
                val curStageHashSet = scheduler.sc.dagScheduler.runningStages.filter(_.id == descrTask.stageId).filter(_.parents.nonEmpty) //.head
                if (curStageHashSet.isEmpty){
                  logDebug("The stage of these tasks has no parents!")
                }
                else if (curStageHashSet.nonEmpty) {

                  //val duration2 = (System.nanoTime - t2) / 1e9d
                  //logInfo(s"Duration of message prep2: $duration2")

                  //val t3 = System.nanoTime
                  if (conf.get(config.DISTRIBUTED_CHERRY_ENABLED)) {
                    /*new Thread("CoarseGrainedSchedulerBackend-send-message-to-Cherry-nodes") {
                      override def run(): Unit = {*/
                    /*scheduler.sc.env.blockManager.cherryShuffleServerIdArray.foreach({ cherryShuffleServerId =>
                      logDebug(s"Sending LaunchTask info to Cherry node Id: $cherryShuffleServerId with host: ${cherryShuffleServerId.host}, port: ${conf.get(config.LOOK_AHEAD_CACHING_PORT)}")
                      val cherryRpcAddress = RpcAddress(cherryShuffleServerId.host, conf.get(config.LOOK_AHEAD_CACHING_PORT))
                      informCherryForLaunchingTask(descrTask.appId.get, descrTask.partitionId, descrTask.stageId, cherryRpcAddress)
                    })*/

                    scheduler.sc.dagScheduler.cherryDataMap.foreach{ case (key, cherryNodeEndpoint) =>
                      informCherryForLaunchingTask(descrTask.appId.get, descrTask.partitionId, descrTask.stageId, cherryNodeEndpoint)
                    }
                    /*}
                  }.start()*/
                    ...
                    //val cherryRpcAddress = RpcAddress(conf.get(config.SHUFFLE_SERVICE_HOST), conf.get(config.LOOK_AHEAD_CACHING_PORT))
                    val cherryEndpoint = rpcEnv.setupEndpointRef(RpcAddress(conf.get(config.SHUFFLE_SERVICE_HOST), conf.get(config.LOOK_AHEAD_CACHING_PORT)), "RemoteShuffleServiceEndpoint")
                    informCherryForLaunchingTask(descrTask.appId.get, descrTask.partitionId, descrTask.stageId, cherryEndpoint)
                  }
                  //val duration3 = (System.nanoTime - t3) / 1e9d
                  //logInfo(s"Duration of message send: $duration3")
                  //val duration1 = (System.nanoTime - t1) / 1e9d
                  //logInfo(s"Duration of message prep + send: $duration1")
                }
                //informCherryForLaunchingTask(new SerializableBuffer(serializedTask), task)


              }
          /*  }
          }
          informCherryThread.setDaemon(true)
          informCherryThread.setName("CoarseGrainedSchedulerBackend-send-message-to-Cherry-nodes")
          informCherryThread.start()*/
  ...
  //def informCherryForLaunchingTask(serializedTask: ByteBuffer, stageId: Int): Unit = {
  //def informCherryForLaunchingTask(taskDescr: SerializableBuffer, task: TaskDescription, cherryRpcAddress: RpcAddress): Unit = {
  def informCherryForLaunchingTask(appId: String, partitionId: Int, stageIdofTask: Int, cherryEndpoint: RpcEndpointRef): Unit = {

    //val taskDescr = new SerializableBuffer(serializedTask)
    logDebug("Informing CHERRY Shuffle Service for launching task and sending new serializedTask Message ")
    try {
      //val rssRpcAddress =  cherryRpcAddress //RpcAddress(conf.get(config.SHUFFLE_SERVICE_HOST),conf.get(config.LOOK_AHEAD_CACHING_PORT))
      //val cherryEndpoint = rpcEnv.setupEndpointRef(cherryRpcAddress, "RemoteShuffleServiceEndpoint")
      // REMOVED rssEndpoint.send(LaunchTaskDescrWithTask(taskDescr, task))
      //cherryEndpoint.send(LaunchTask(taskDescr))
      cherryEndpoint.send(LaunchTaskForCherry(appId, partitionId, stageIdofTask))
    } catch {
      case e: Exception =>
        throw new SparkException("Error informing RSS for launching task", e)
    }
