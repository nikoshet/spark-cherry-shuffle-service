...
private[master] class MasterArguments(args: Array[String], conf: SparkConf) extends Logging {
  ...
  if (System.getenv("SPARK_MASTER_PORT") != null) {
    if (System.getenv("SPARK_MASTER_PORT").contains(":")){
      port = System.getenv("SPARK_MASTER_PORT").substring(System.getenv("SPARK_MASTER_PORT").lastIndexOf(':') + 1).toInt
    }
    else{
  ...
  if (System.getenv("SPARK_MASTER_WEBUI_PORT") != null) {
    if (System.getenv("SPARK_MASTER_WEBUI_PORT").contains(":")){
      webUiPort = System.getenv("SPARK_MASTER_WEBUI_PORT").substring(System.getenv("SPARK_MASTER_WEBUI_PORT").lastIndexOf(':') + 1).toInt
    }
    else{
