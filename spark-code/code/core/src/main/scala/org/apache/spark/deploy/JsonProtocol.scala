...
import org.apache.spark.deploy.DeployMessages.{MasterStateResponse, ShuffleServiceStateResponse, WorkerStateResponse}
...
private[deploy] object JsonProtocol {
  ...
  /**
   * Export the [[ShuffleServiceStateResponse]] to a Json object. A [[ShuffleServiceStateResponse]] consists the
   * information of a shuffle service + block handler.
   *
   * @return a Json object containing the following fields:
   *  to be added..
   *
   */
  def writeShuffleServiceState(obj: ShuffleServiceStateResponse): JObject = {
    ("host" -> obj.host) ~
    ("port" -> obj.port) ~
    ("blockTransferRateBytes" -> obj.blockTransferRateBytes) ~
    ("registeredExecutorsSize" -> obj.registeredExecutorsSize) ~
    ("numActiveConnections" -> obj.numActiveConnections) ~
    ("numCaughtExceptions" -> obj.numCaughtExceptions)
  }
}
