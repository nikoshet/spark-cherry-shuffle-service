...
private[ui] class LogPage(parent: ShuffleServiceWebUI) extends WebUIPage("logPage") with Logging {
  private val shuffleService = parent.shuffleService
  private val workDir = new File(parent.shuffleService.workDir.toURI.normalize().getPath)
  ...
  private def getLog(
    ...
    try {
      ...
      val fileLengths: Seq[Long] = files.map(Utils.getFileLength(_, shuffleService.sparkConf))
